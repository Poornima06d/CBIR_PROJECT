...
# ✅ Load ResNet50 (same model as used in preprocessing)
model = models.resnet50(weights="IMAGENET1K_V1")
model = nn.Sequential(*list(model.children())[:-1])
model.eval()

# ✅ Image transformation (same as in preprocess)
...
# === 🖼️ Main Logic ===
print("\n💡 You can give any image path (e.g., from Downloads, Desktop, or external device)")
query_path = input("Enter the path of query image: ").strip()

if not os.path.exists(query_path):
    print("❌ Image not found. Please check your path.")
    exit()
...
print("\n🔍 Top", top_n, "most similar images found:\n")
for i, idx in enumerate(sorted_indices):
    print(f"{i+1}. {image_paths[idx]} (similarity: {similarities[idx]:.4f})")

# === 📥 Download Option (moved before feedback) ===
...
# === ⭐ Feedback Section (after download) ===
feedback = input("\nWould you like to give feedback for these results? (y/n): ").strip().lower()
if feedback == "y":
